...
class AgentBase:
    def __init__(self, net_dims: [int], state_dim: int, action_dim: int, gpu_id: int = 0, args: RLConfig = RLConfig()):
        ...
        self.act = self.act_target = act_class(net_dims, state_dim, action_dim,args=args).to(self.device)
        self.cri = self.cri_target = cri_class(net_dims, state_dim, action_dim, args=args).to(self.device) \
    ...
    def update_avg_std_for_normalization(self, states: Tensor, returns: Tensor):
        ...
        state_avg = states.mean(dim=-1, keepdim=True).mean(dim=0, keepdim=False)
        state_std = states.std(dim=-1, keepdim=True).mean(dim=0, keepdim=False)
