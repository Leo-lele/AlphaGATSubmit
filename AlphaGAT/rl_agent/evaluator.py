...
class Evaluator:
    ...
    def evaluate_and_save(self, actor: torch.nn, steps: int, exp_r: float, logging_tuple: tuple):
        ...
        test_return_step = rewards_step_ten[2]


        train_time = int(time.time() - self.start_time)

        '''record the training information'''
        self.recorder.append((self.total_step, train_return_step[0], valid_return_step[0], test_return_step[0] , exp_r, *logging_tuple))  # update recorder
        if self.tensorboard:
            ...
            self.tensorboard.add_scalar("test/train_reward_sample", train_return_step[0], self.total_step)
            self.tensorboard.add_scalar("test/valid_reward_sample", valid_return_step[0], self.total_step)
            self.tensorboard.add_scalar("test/test_reward_sample", test_return_step[0], self.total_step)
            self.tensorboard.add_scalar("test/train_reward_time", train_return_step[0], train_time)
            self.tensorboard.add_scalar("test/valid_reward_time", valid_return_step[0], train_time)
            self.tensorboard.add_scalar("test/test_reward_time", test_return_step[0], train_time)



        avg_r = test_return_step[0]
        ...
        print(f"{self.agent_id:<3}{self.total_step:8.2e}{train_time:8.0f} |"
              f"{train_return_step[0]:8.4f}{valid_return_step[0]:8.4f}{test_return_step[0]:8.4f}|"
    ...
    def save_Figs_end(self):
        ...
        plt.plot(recorder[:,0], recorder[:,3], label='Test', color="green")
